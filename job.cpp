...
Job::Job(const std::string &jobId, const std::string &jobTitle, const std::string &companyId, const std::string &description)
    : Job_ID(jobId), Job_Title(jobTitle), Company_ID(companyId), Description(description) {
    std::cout << "Job1 created" << std::endl;
}

void Job::addApplicant(const Person &person) {
    applicants.push_back(person);
}

bool Job::saveToDatabase(QSqlDatabase& db) const {
    QSqlQuery query(db);
...
bool Job::loadFromDatabase(const std::string &id, QSqlDatabase& db) {
    QSqlQuery query(db);
